...
#input001=/home/melodies-ist/data/ISD/ISD000/SM001/Sx001_.tif
#input002=/home/melodies-ist/data/ISD/ISD000/VITO/LANDC001_1.tif

# => For Discriminant classes 0.8:
for i in {4,5,6,7}; do 
gdal_calc.py -A $input001 -B $input002 --outfile=$PDIR/CR_0$i.tif --calc="(B==$i)*(A*0+8000)" --overwrite  --NoDataValue=-9999 --type=Int32;
done

# => For Non-Discriminant classes 0.5:
for i in {1,8,9,10,11}; do 
gdal_calc.py -A $input001 -B $input002 --outfile=$PDIR/CR_0$i.tif --calc="(B==$i)*(A*0)" --overwrite  --NoDataValue=-9999 --type=Int32;
done
...
#-------------------------------------------------------------------------------------#
export -p PDIR=$OUTDIR/PM001
mv $PDIR/CR_010.tif  $PDIR/CR_10.tif
mv $PDIR/CR_011.tif  $PDIR/CR_11.tif
...
list.filenames=assign(paste("list.filenames_",sep=""),list.files(pattern=paste("CR_",".*\\.tif",sep="")))
list.filenames
...
summary(raster(list.filenames[1]))
summary(raster(list.filenames[2])) 
summary(raster(list.filenames[3])) 
summary(raster(list.filenames[4]))
summary(raster(list.filenames[5]))
summary(raster(list.filenames[6])) 
summary(raster(list.filenames[7]))
summary(raster(list.filenames[8]))
summary(raster(list.filenames[9]))
summary(raster(list.filenames[10]))
summary(raster(list.filenames[11]))
