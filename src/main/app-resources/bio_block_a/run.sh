...
#rm -rf /data/outDIR/ISD/ISD000/SM001/
#rm -rf /data/outDIR/ISD/ISD000/VM001/
...
export -p NVDIR=$VDIR/class_NDV001/ndv_mosaic && mkdir -pm 777 $NVDIR
export -p SBDIR=$CDIR/class_SOIL001/soil_mosaic && mkdir -pm 777 $SBDIR

function igcx(){
exec $IXDIR"vgt_to_geoms_00201.sh" $Y2 $IR & 
wait
exec $IXDIR"vgt_to_geoms_00203.sh" $Y2 $IR & 
wait
exec $IXDIR"vgt_to_geoms_00101.sh" $Y2 $IR & 
wait
exec $IXDIR"vgt_to_geoms_00103.sh" $Y2 $IR &  
wait
